...
public class PostServiceImpl implements PostService {
    ...
    public PostResponse updatePost(Long postId, PostRequest postRequest) {
        ...
        return new PostResponse(updatedPost.getId(), updatedPost.getTitle(), updatedPost.getDescription(), updatedPost.getCreatedAt().toString());
    }
    ...
    public List<PostResponse> getUserFeed(Long userId) {
        ...
        List<Post> posts = postRepository.findAll().stream()
                .filter(post -> post.getUser().equals(user) || user.getFollowers().contains(post.getUser()))  // Include user and their followers
                .sorted((p1, p2) -> p2.getCreatedAt().compareTo(p1.getCreatedAt()))  // Sort by latest first
                .collect(Collectors.toList());
